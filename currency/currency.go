...
func GetFactor(from, to string) (factor float64, err error) {
	if time.Now().After(lastTime) {
		...
		for name, valute := range list.Valutes {
			valutes.Store(name, valute)
		}
	}
	...
	fromIface, ok := valutes.Load(from)
	if !ok {
	...
	fromValute := fromIface.(Valute)

	toIface, ok := valutes.Load(to)
	if !ok {
	...
	toValute := toIface.(Valute)

	factor = (fromValute.Value / float64(fromValute.Nominal)) / (toValute.Value / float64(toValute.Nominal))
